use super::Lump;
use crate::WadString;
use crate::lump_parser::{LumpParser, ParseError, Result};

pub use colormap::ColormapLump;
pub use flat::FlatLump;
pub use patch::PatchLump;
pub use playpal::PlaypalLump;
pub use pnames::PnamesLump;
pub use textures::TexturesLump;

pub mod playpal {
	use super::*;

	#[derive(Debug)]
	pub struct PlaypalLump {
		pub palettes: Vec<playpal::Palette>,
	}

	impl Lump for PlaypalLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let mut palettes = Vec::with_capacity(data.len() / playpal::Palette::SIZE);
			while parser.has_data_left() {
				let bytes = parser.read_chunk::<{ playpal::Palette::SIZE }>()?;
				palettes.push(playpal::Palette::from_bytes(&bytes));
			}

			// has_data_left() must be false, so no need to finish()

			Ok(PlaypalLump { palettes })
		}
	}

	#[derive(Debug)]
	pub struct Palette {
		pub colors: [Color; Self::NUM_COLORS],
	}

	impl Palette {
		pub const NUM_COLORS: usize = 256;
		pub const SIZE: usize = Self::NUM_COLORS * Color::SIZE;

		pub fn from_bytes(bytes: &[u8; Self::SIZE]) -> Self {
			let (chunks, _) = bytes.as_chunks::<3>();

			Self {
				colors: chunks
					.iter()
					.map(Color::from_bytes)
					.collect::<Vec<_>>()
					.try_into()
					.unwrap(),
			}
		}
	}

	#[derive(Debug, PartialEq)]
	pub struct Color {
		pub r: u8,
		pub g: u8,
		pub b: u8,
	}

	impl Color {
		pub const SIZE: usize = 3;

		pub fn from_bytes(data: &[u8; Self::SIZE]) -> Self {
			Self {
				r: data[0],
				g: data[1],
				b: data[2],
			}
		}
	}

	// Implemented to make testing easier
	impl PartialEq<[u8; 3]> for Color {
		fn eq(&self, other: &[u8; 3]) -> bool {
			[self.r, self.g, self.b] == *other
		}
	}
}

pub mod colormap {
	use super::*;

	#[derive(Debug)]
	pub struct ColormapLump {
		pub maps: [[u8; playpal::Palette::NUM_COLORS]; Self::NUM_MAPS],
	}

	impl ColormapLump {
		pub const NUM_MAPS: usize = 34;
		pub const INVULN_INDEX: usize = 32;
	}

	impl Lump for ColormapLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let maps: Vec<_> = (0..Self::NUM_MAPS)
				.map(|_| parser.read_chunk::<{ playpal::Palette::NUM_COLORS }>())
				.collect::<Result<_>>()?;

			parser.finish()?;

			Ok(Self {
				maps: maps.try_into().unwrap(),
			})
		}
	}
}

pub mod textures {
	use super::*;

	/// The TEXTURE1 and TEXTURE2 lumps.
	#[derive(Debug)]
	pub struct TexturesLump {
		pub num_textures: i32,
		pub offsets: Vec<i32>,
		pub textures: Vec<textures::TexEntry>,
	}

	impl Lump for TexturesLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let num_textures = parser.read_i32()?;

			let offsets: Vec<i32> = (0..num_textures)
				.map(|_| parser.read_i32())
				.collect::<Result<_>>()?;

			// TODO: Evaluate the best parsing strategy here. There are a few ways you
			//  could go about this, depending on the assumptions you make about WADs.
			//  Mainly, are the TexEntries always contiguous in the lump?
			let textures: Vec<textures::TexEntry> = offsets
				.iter()
				.map(|offset| textures::TexEntry::parse(&data[(*offset as usize)..]))
				.collect::<Result<_>>()?;

			Ok(Self {
				num_textures,
				offsets,
				textures,
			})
		}
	}

	#[derive(Debug)]
	pub struct TexEntry {
		pub name: WadString,
		pub _masked: i32,
		pub tex_width: i16,
		pub tex_height: i16,
		pub _columndirectory: i32,
		pub num_patches: i16,
		pub patches: Vec<Patch>,
	}

	impl TexEntry {
		pub fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let name = WadString::from_bytes(parser.read_chunk::<8>()?)
				.map_err(ParseError::InvalidString)?;
			let _masked = parser.read_i32()?;
			let tex_width = parser.read_i16()?;
			let tex_height = parser.read_i16()?;
			let _columndirectory = parser.read_i32()?;
			let num_patches = parser.read_i16()?;

			let patches: Vec<Patch> = (0..num_patches)
				.map(|_| {
					let bytes = parser.read_chunk::<10>()?;
					Ok(Patch::from_bytes(&bytes))
				})
				.collect::<Result<_>>()?;

			// For now we intentionally don't call finish()

			Ok(Self {
				name,
				_masked,
				tex_width,
				tex_height,
				_columndirectory,
				num_patches,
				patches,
			})
		}
	}

	#[derive(Debug)]
	pub struct Patch {
		pub x_offset: i16,
		pub y_offset: i16,
		pub pname_index: i16,
		pub _stepdir: i16,
		pub _colormap: i16,
	}

	impl Patch {
		const NUM_SIZE: usize = 10;

		pub fn from_bytes(bytes: &[u8; Self::NUM_SIZE]) -> Self {
			Self {
				x_offset: i16::from_le_bytes(bytes[0..2].try_into().unwrap()),
				y_offset: i16::from_le_bytes(bytes[2..4].try_into().unwrap()),
				pname_index: i16::from_le_bytes(bytes[4..6].try_into().unwrap()),
				_stepdir: i16::from_le_bytes(bytes[6..8].try_into().unwrap()),
				_colormap: i16::from_le_bytes(bytes[8..10].try_into().unwrap()),
			}
		}
	}
}

pub mod pnames {
	use super::*;

	#[derive(Debug)]
	pub struct PnamesLump {
		pub pnames: Vec<WadString>,
	}

	impl Lump for PnamesLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let num_patches = parser.read_i32()?;

			let pnames: Vec<WadString> = (0..num_patches)
				.map(|_| {
					WadString::from_bytes(parser.read_chunk::<8>()?)
						.map_err(ParseError::InvalidString)
				})
				.collect::<Result<_>>()?;

			parser.finish()?;

			Ok(Self { pnames })
		}
	}
}

pub mod patch {
	use super::*;

	/// Doom's transparency-supporting picture format used for textures, sprites, and HUD graphics.
	#[derive(Debug)]
	pub struct PatchLump {
		pub width: u16,
		pub height: u16,
		pub x_offset: i16,
		pub y_offset: i16,
		pub columns: Vec<patch::Column>,
	}

	impl Lump for PatchLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);

			let width = parser.read_u16()?;
			let height = parser.read_u16()?;
			let x_offset = parser.read_i16()?;
			let y_offset = parser.read_i16()?;

			let column_offsets: Vec<u32> = (0..width)
				.map(|_| parser.read_u32())
				.collect::<Result<_>>()?;

			let columns = column_offsets
				.into_iter()
				.map(|offset| &data[offset as usize..])
				.map(patch::Column::parse)
				.collect::<Result<_>>()?;

			Ok(Self {
				width,
				height,
				x_offset,
				y_offset,
				columns,
			})
		}
	}

	#[derive(Debug)]
	pub struct Column {
		pub posts: Vec<Post>,
	}

	impl Column {
		pub fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);
			let mut posts = Vec::new();

			loop {
				// The column ends when the next post starts with 0xff
				let y_offset = parser.read_u8()?;
				if y_offset == 0xff {
					break;
				}

				let length: usize = parser.read_u8()?.into();

				parser.read_padding(1)?;

				posts.push(patch::Post {
					y_offset,
					data: parser.read_slice(length)?.to_vec(),
				});

				parser.read_padding(1)?;
			}

			Ok(patch::Column { posts })
		}
	}

	#[derive(Debug)]
	pub struct Post {
		pub y_offset: u8,
		pub data: Vec<u8>,
	}
}

/// Doom's flat (floor) graphic format.
pub mod flat {
	use super::*;

	#[derive(Debug)]
	pub struct FlatLump {
		pub data: [u8; 4096],
	}

	impl Lump for FlatLump {
		fn parse(data: &[u8]) -> Result<Self> {
			let mut parser = LumpParser::new(&data);
			let data = parser.read_chunk::<4096>()?;
			parser.finish()?;

			Ok(FlatLump { data })
		}
	}
}

#[cfg(test)]
mod tests {
	use super::*;

	#[test]
	fn playpal_color_from_bytes() {
		let color = playpal::Color::from_bytes(&[255, 159, 67]);
		assert_eq!(color.r, 255);
		assert_eq!(color.g, 159);
		assert_eq!(color.b, 67);
	}
}
